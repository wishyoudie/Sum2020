...
VOID VI6_RndPrimDraw( vi6PRIM *Pr, MATR World )
{
  ...
  for (i = 0; i < Pr->NumOfV; i++)
  {
    ...
    pnts[i].x = (INT)((p.X + 1) * VI6_RndFrameW / 2);
    pnts[i].y = (INT)((-p.Y + 1) * VI6_RndFrameH / 2);
  }

  /* Draw triangles */
  for (i = 0; i < Pr->NumOfI; i += 3)
  {
    MoveToEx(VI6_hRndDCFrame, pnts[Pr->I[i]].x, pnts[Pr->I[i]].y, NULL);
    LineTo(VI6_hRndDCFrame, pnts[Pr->I[i + 1]].x, pnts[Pr->I[i + 1]].y);
    LineTo(VI6_hRndDCFrame, pnts[Pr->I[i + 2]].x, pnts[Pr->I[i + 2]].y);
    LineTo(VI6_hRndDCFrame, pnts[Pr->I[i]].x, pnts[Pr->I[i]].y);
  }
  free(pnts);
